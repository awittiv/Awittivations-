"""
Live Aave v3 Polygon rates via direct Polygon RPC — no API key, no cost.
...
def _fetch_rates_sync() -> dict[str, dict]:
...
async def fetch_aave_rates() -> dict[str, dict]:
    """
    ...
    with ThreadPoolExecutor(max_workers=1) as pool:
        return await loop.run_in_executor(pool, _fetch_rates_sync)
